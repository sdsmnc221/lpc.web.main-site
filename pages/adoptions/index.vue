    ...
    childrenNodes.forEach((child, index) => {
      const st = ScrollTrigger.create({
        ...
        onUpdate: self => {
          ...
          gsap.to(child, {
            y: gsap.utils.interpolate(0, 240 * (0.2 * (itemIndex + 1)), progress),
            x: gsap.utils.interpolate(0, -32 * (0.2 * (itemIndex + 1)), progress),
            duration: 0.1,
            overwrite: true
          });
        }
...
@media (pointer: fine) {
  .adoptions-group {
    // Scrollbars visibles sur desktop pour une meilleure UX
    &::-webkit-scrollbar {
      height: 8px;
      width: 8px;
    }
